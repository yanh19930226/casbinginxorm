...
func (s *Set) validate(i interface{}) {
	...
	case int:
		if s.tp != intType {
			log.Fatal("Error: element is int, but set contains elements with type %d", s.tp)
		}
	case int64:
		if s.tp != int64Type {
			log.Fatal("Error: element is int64, but set contains elements with type %d", s.tp)
		}
	case uint:
		if s.tp != uintType {
			log.Fatal("Error: element is uint, but set contains elements with type %d", s.tp)
		}
	case uint64:
		if s.tp != uint64Type {
			log.Fatal("Error: element is uint64, but set contains elements with type %d", s.tp)
		}
	case string:
		if s.tp != stringType {
			log.Fatal("Error: element is string, but set contains elements with type %d", s.tp)
		}
	}
}
